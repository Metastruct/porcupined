...
if not lib.SteamAPI_Init() then
	-- depending on what you do it's okay to error here...
...
steamworks.pipe_ptr = ffi.new("SteamWorks_HSteamPipe[1]")


--g_hUser = g_pSteamClient->CreateLocalUser(&g_hPipe, k_EAccountTypeIndividual);
steamworks.steam_user_ptr = lib.SteamAPI_ISteamClient_CreateLocalUser(steamworks.client_ptr, steamworks.pipe_ptr, assert(lib.k_EAccountTypeIndividual))
if steamworks.steam_user_ptr == nil then error("SteamAPI_ISteamClient_CreateLocalUser() returns NULL") end

--g_user = g_pSteamClient->GetISteamUser(g_hUser, g_hPipe, "SteamUser020");


steamworks.user = {}
steamworks.user_ptr = lib.SteamAPI_ISteamClient_GetISteamUser(steamworks.client_ptr, steamworks.steam_user_ptr, steamworks.pipe_ptr[0], 'SteamUser019')
